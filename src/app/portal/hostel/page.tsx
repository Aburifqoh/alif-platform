...
export default async function HostelDashboardPage() {
  ...
  const statusConfig = {
    ...
    approved:     { label: "Approved",        color: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-950/30", icon: CheckCircle2 },
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      {!isResident && application && (() => {
        ...
        return (
          <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-white/8 p-5 flex items-center gap-4">
            ...
            <Link href="/portal/hostel/application" className="text-sm text-[#0f5132] dark:text-emerald-400 flex items-center gap-1 flex-shrink-0">
        ...
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-white/8 overflow-hidden">
          <div className="p-5 sm:p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className="px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 text-xs font-semibold">
                Active
              </span>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-white/8 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/portal/hostel/payments" className="text-sm text-[#0f5132] dark:text-emerald-400 flex items-center gap-1">
              View all <ChevronRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          {(unpaidFees ?? []).length === 0 ? (
            <div className="text-center py-8">
              <CheckCircle2 className="w-10 h-10 mx-auto mb-2 text-emerald-300 dark:text-emerald-700" />
              ...
              {unpaidFees!.slice(0, 3).map((fee) => (
                <div key={fee.id} className="flex items-center justify-between p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30">
                  ...
                  <div className="text-right">
                    ...
                    <Link href="/portal/hostel/payments" className="text-xs text-[#0f5132] dark:text-emerald-400 hover:underline">
                      Pay now →
                    </Link>
                  </div>
                </div>
        ...
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-white/8 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/portal/hostel/maintenance" className="text-sm text-[#0f5132] dark:text-emerald-400 flex items-center gap-1">
              View all <ChevronRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          {(openMaintenance ?? []).length === 0 ? (
            <div className="text-center py-8">
              ...
              <Link href="/portal/hostel/maintenance" className="mt-2 inline-block text-sm text-[#0f5132] dark:text-emerald-400 hover:underline">
                Submit a request →
              </Link>
            </div>
          ) : (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { icon: CalendarDays, label: "Apply / Renew", href: "/portal/hostel/apply", color: "from-emerald-600 to-emerald-700" },
