...
export default async function ApplicationStatusPage() {
      ...
      <div className="p-4 sm:p-6 lg:p-8 flex items-center justify-center min-h-[60vh]">
        <div className="max-w-sm w-full text-center">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-[#0f5132] to-[#166534] text-white font-semibold rounded-xl text-sm font-[Outfit]"
          >
  ...
  const statusBadge = {
    ...
    approved:     { label: "Approved ✓",   color: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-950/30" },
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-2xl">
      ...
      <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-white/8 overflow-hidden">
                  ...
                  <div key={s.key} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0
                        ${done ? "bg-[#0f5132] text-white" : active ? "bg-[#0f5132] text-white ring-4 ring-emerald-100 dark:ring-emerald-950" : "bg-gray-100 dark:bg-white/8 text-gray-400"}`}>
                        {done ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs font-bold">{i + 1}</span>}
                      </div>
                      {i < STATUS_STEPS.length - 1 && (
                        <div className={`w-0.5 h-10 mt-1 ${done ? "bg-[#0f5132]" : "bg-gray-100 dark:bg-white/8"}`} />
                      )}
                    </div>
                    <div className="pb-8">
        ...
        {isRejected && (
          <div className="p-5 sm:p-6">
            ...
            <Link
              href="/portal/hostel/apply"
              className="mt-4 inline-flex items-center gap-2 text-sm text-[#0f5132] dark:text-emerald-400 hover:underline font-medium"
            >
      ...
      {isApproved && application.allocated_room && (
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-emerald-200 dark:border-emerald-900 p-5 sm:p-6">
      ...
      {/* Pay now CTA */}
      {isApproved && (
        <div className="bg-gradient-to-br from-[#0f5132] to-[#166534] rounded-2xl p-6 flex items-center justify-between gap-4">
          <div>
