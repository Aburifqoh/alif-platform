      ...
      <div className="p-4 sm:p-6 lg:p-8 flex items-center justify-center min-h-[60vh]">
        <div className="max-w-md w-full text-center">
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-[#0f5132] to-[#166534] text-white font-semibold rounded-xl font-[Outfit]"
          >
            Track Application
          </button>
        </div>
            ...
            <div className="flex flex-col items-center">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all
                  ${i < step ? "bg-[#0f5132] text-white" : i === step ? "bg-[#0f5132] text-white ring-4 ring-emerald-100 dark:ring-emerald-950" : "bg-gray-100 dark:bg-white/8 text-gray-400"}`}
              >
                {i < step ? <CheckCircle2 className="w-4 h-4" /> : i + 1}
              </div>
              <span className={`text-xs mt-1 font-medium whitespace-nowrap ${i === step ? "text-[#0f5132] dark:text-emerald-400" : "text-gray-400"}`}>
                {s}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`w-12 sm:w-20 h-0.5 mx-1 -mt-4 ${i < step ? "bg-[#0f5132]" : "bg-gray-200 dark:bg-white/10"}`} />
            )}
          </div>
        ))}
      </div>
                  ...
                  <button
                    ...
                    className={`py-2.5 rounded-xl text-sm font-medium capitalize border transition-all
                      ${form.purpose === p
                        ...
                        : "border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-[#0f5132] dark:hover:border-emerald-500"
                      }`}
                  >
                    {p}
                  </button>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-xl border text-left transition-all
                      ${form.hostel_slug === opt.value
                        ? "bg-emerald-50 dark:bg-emerald-950/20 border-[#0f5132] dark:border-emerald-600"
                        : "border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20"
                      }`}
                  >
                    <div>
                      <div className={`text-sm font-semibold ${form.hostel_slug === opt.value ? "text-[#0f5132] dark:text-emerald-400" : "text-gray-900 dark:text-white"}`}>{opt.label}</div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  >
                    <div>
                      <div className={`text-sm font-semibold ${form.room_type === opt.value ? "text-[#0f5132] dark:text-emerald-400" : "text-gray-900 dark:text-white"}`}>{opt.label}</div>
                      <div className="text-xs text-gray-400 mt-0.5">{opt.desc}</div>
                    </div>
                    <span className={`text-sm font-bold ${form.room_type === opt.value ? "text-[#0f5132] dark:text-emerald-400" : "text-gray-500"}`}>{opt.fee}</span>
                  </button>
                ))}
              </div>
            </div>
              ...
              return (
                <div key={doc.type}>
                  ...
                  {uploaded ? (
                    ...
                    <label className="flex flex-col items-center gap-2 p-6 rounded-xl border-2 border-dashed border-gray-200 dark:border-white/10 cursor-pointer hover:border-[#0f5132] dark:hover:border-emerald-600 transition-colors">
        ...
        <div className="flex items-center justify-between mt-8">
          ...
          {step < STEPS.length - 1 ? (
            <button
              type="button"
              onClick={() => setStep((s) => s + 1)}
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#0f5132] to-[#166534] text-white text-sm font-semibold hover:shadow-lg hover:-translate-y-0.5 transition-all font-[Outfit]"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#0f5132] to-[#166534] text-white text-sm font-semibold hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-60 transition-all font-[Outfit]"
            >
              {loading ? (
